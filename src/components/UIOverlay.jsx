...
export default function UIOverlay() {
  const { activeAlbum, status, closeAlbum } = useAlbumStore()
  const [progress, setProgress] = useState(0)

  useEffect(() => {
    if (status === 'playing' && activeAlbum) {
    ...
    <div style={styles.overlay}>
        ...
        {activeAlbum && (
          <>
            <div style={styles.statusBadge}>
              <span style={{ ...styles.dot, background: status === 'playing' ? '#b8865a' : 'rgba(20,20,20,0.3)' }} />
              <span style={styles.statusText}>
                {status === 'browse' ? 'Browse' : status === 'focus' ? 'Selected' : status === 'open' ? 'Open' : 'Playing'}
              </span>
            </div>
            <button
              onClick={closeAlbum}
              style={styles.closeBtn}
              onMouseEnter={(e) => { e.target.style.borderColor = 'rgba(240,236,228,0.6)'; e.target.style.color = '#141414' }}
              onMouseLeave={(e) => { e.target.style.borderColor = 'rgba(20,20,20,0.2)'; e.target.style.color = 'rgba(20,20,20,0.5)' }}
            >
              Close
            </button>
          </>
      ...
      {!activeAlbum && (
        <div style={styles.hint}>
          <p style={{ fontSize: '11px', letterSpacing: '0.25em', textTransform: 'uppercase', color: 'rgba(20,20,20,0.25)', margin: 0 }}>
            ...
            <div style={styles.player}>
              ...
              <div style={styles.playerControls}>
                <span style={styles.time}>
                  {formatTime(progress, activeAlbum.duration)} / {activeAlbum.duration}
                </span>
              </div>
            </div>
          )}
